...
/**
 * A <code>Playground</code> is the <code>Turtle</code>'s home, i.e. the
 * <code>Turtle</code> lives and moves in the <code>Playground</code>.
 * 
 * The<code>Playground</code> is responsible for interpreting angle and position
 * of the <code>Turtle</code> and for choosing the correct turtle image and
 * putting it on the right spot of the Playground. This means: e.g. whenever you
 * wish to switch the x- and y-axis, you should do it in this class, and not in
 * the <code>Turtle</code> class. <i>Remarks:</i>
 * <ul>
 * <li>The <code>Playground</code> needs a Window (e.g. <code>Frame</code>,
 * <code>Applet</code>) to be displayed. One possible solution for this problem
 * is already given with the <code>TurtleFrame</code> class, which implements
 * the <code>TurtleContainer</code> interface. Applets are not yet
 * supported.</li>
 * <li>The <code>Turtle</code> itself has only direct access to its
 * <code>Playground</code>, but not to the <code>TurtleContainer</code>.</li>
 * </ul>
 * 
 * 
...
public class Playground extends JPanel implements ImageObserver {
  /* ********** ATTRIBUTES ********** */
  ...
  Image turtleBuffer;
  /**
   * This is the window (or applet or ...) which holds this
   ...
   * with standard size (400 x 400 pixels).
   */
  public Playground(TurtleContainer rootpane) {
    super();
    init(rootpane, new Dimension(1000, 700));
  }
   ...
   * 
   * e.g. creates a new vector (which holds the <code>Turtle</code>s), the
   * offscreen buffers, and sets the size and background color.
   * 
  ...
  protected void init(TurtleContainer rootpane, Dimension size) {
    ...
    turtles = new Vector<>();
    canvasBuffer = new BufferedImage(size.width, size.height, BufferedImage.TYPE_INT_ARGB);
  ...
  /**
   * Moves the given <code>Turtle</code> above all the others, then paints all
   * turtles.
   * 
   * @see #toTop
   */
  ...
  void paintTurtle(Turtle turtle) {
    ...
    turtle.getTurtleRenderer().paint(turtle.getX(), turtle.getY(), turtleGraphics);
  ...
  void stampTurtle(Turtle turtle) {
    ...
    turtle.getTurtleRenderer().paint(turtle.getX(), turtle.getY(), canvasGraphics);
  ...
  void lineTo(double x0, double y0, double x1, double y1, Pen pen) {
    ...
    RepaintManager.currentManager(this).addDirtyRegion(this, ix0, iy0, ix1 - ix0, iy1 - iy0);
    offGraphics.dispose();
   ...
   * Fills a region.
   * 
   * The region is defined by the <code>Turtle</code>s actual position and is
   * bounded by any other color than the background color.<br>
        ...
        while (d <= S) { // forward
          ...
          try {
            if (((BufferedImage) canvasBuffer).getRGB(tmp.x, tmp.y) == bgcolor) {
              ...
              list.addElement(Integer.valueOf(d));
              d = N;
            } else {
              d++;
            }
  ...
  /**
   * Draws part of canvas and turtle buffers into the offscreen buffer, according
   * to the rectangle definded by the parameters.
   * 
  ...
  /**
   * Draws part (a rectangle defined by the parameters) of an image into the
   * offscreen buffer.
   * 
   * @param buffer The image to be drawn.
   * @param x      the rectangle's x-coordinate
   ...
   * @param h      the rectangle's height
   */
  public void blitToOffscreenBuffer(Image buffer, int x, int y, int w, int h) {
   ...
   * Only override this method if you have added a new behaviour (like wrap or
   * clip) to the turtle.
   */
  ...
  /**
   * Here the actual clearing of a <code>Turtle</code> in wrap mode from the given
   * image is performed.
   */
   ...
   */
  public Rectangle copyAndTranslate(Rectangle rect, int dx, int dy) {
  ...
  /**
   * Here the actual clearing of a <code>Turtle</code> in clip mode from the given
   * image is performed.
   */
  ...
  /**
   * Sets the alpha channel of all pixels in the given image in the given
   * Rectangle to zero (i.e. totally transparent).
   * 
  ...
  Image getOffscreenBuffer() {
    Image off = getRepaintManager().getOffscreenBuffer(this, getWidth(), getHeight());
    return off;
  }
   ...
   * Cleans the Canvas.
   * 
   * All turtles stay how and where they are, only lines, text and stamps will be
   * removed.
   */
  void clean() {
  ...
  /**
   * Draws the <code>text</code> at the current position of the Turtle
   * <code>t</code>.
   * 
   * Drawing a text at some coordinates <code>(x,y)</code> we mean that the bottom
   * left corner of the text will be at these coordinates. Font and colour are
   * specified by the Turtle's Pen.
   */
