...
public class Turtle {
   ...
   * 
   * It specifies how many different turtle pictures are generated.
   */
  ...
  /**
   * Create a new <code>Turtle</code> with specified <code>color</code> in its own
   * new Window.
   */
  ...
  /**
   * Create a new <code>Turtle</code> and puts it into the (already existing)
   * <code>turtleContainer</code>
   */
  ...
  /**
   * Create a new <code>Turtle</code> with specified <code>color</code> in the
   * specified <code>turtleContainer</code>.
   */
  ...
  /**
   * Create a new <code>Turtle</code> in the same <code>TurtleContainer</code>
   * (Window) as <code>otherTurtle</code>
   */
  ...
  /**
   * Create a new <code>Turtle</code> with the specified <code>color</code> in the
   * same <code>TurtleContainer</code> (Window) as <code>otherTurtle</code>
   */
   ...
   * 
   * If you want to inherit from this class and add new Turtle attributes, you
   * might want to overwrite this method.
   */
   ...
   * 
   * It specifies how many pictures are used. e.g. an angle resolution of 90 means
   * that you get one picture for every 4 degrees (= 360/90 degrees).
   * 
   ...
   * @see TurtleFactory
   */
  public static TurtleFactory getTurtleFactory() {
    return turtleFactory;
  }

  /**
   * Create a <code>LineRenderer</code> which is responsible for the correct
  ...
  /**
   * Create a <code>TurtleRenderer</code> which is responsible for the correct
   * drawing of the <code>Turtle</code>.
   * 
  ...
  /**
   * Create a <code>TurtleFactory</code> which provides for the Turtle pictures.
   * 
  ...
  /**
   * Set the <code>Playground</code> to the specified <code>playground</code>.
   * 
   * The <code>Turtle</code> is removed from the old <code>Playground</code> and
   * set to the new one.
   */
   ...
   * Get the angle speed.
   * 
   * (I.e. how fast the <code>Turtle</code> rotation animation is performed.)
   */
  ...
  /**
   * get the current angle (heading) of the <code>Turtle</code>.
   */
  ...
  /**
   * Sets the x-coordinate of the <code>Turtle</code>s position to the given
   * value.
   * 
   * @return the turtle to allow chaining.
   */
  ...
  /**
   * Sets the y-coordinate of the <code>Turtle</code>s position to the given
   * value.
   * 
   * @return the turtle to allow chaining.
   */
   ...
   * 
   * @return <code>true</code> if the turtle is hidden, <code>false</code>
   *         otherwise.
   */
  ...
  /**
   * Only set the angle attribute. This method does not invoke any re-painting.
   */
   ...
   * 
   * If you try to set the speed to 0, it will be set to 1 (very slow). A negative
   * speed means that moving the turtle (fd, bk) will not be animated.<br>
   * The unit is pixels per second (up to certain bounds depending on the CPU
   * etc.).<br>
  ...
  /**
   * This is the internal speed setting method. It sets the speed to the specified
   * value.
   */
  ...
  private void internalRotate(double angle) {
    ...
    if (angle != 0) {
      ...
      for (int index = 0; index < iterations; index++) {
        ...
        Double secs = 1000. / getFramesPerSecond();
    ...
    if (getSpeed() > 0) {
      if (length != 0) {
        ...
        for (int index = 0; index < iterations; index++) {
          ...
          if (nX != (int) getX() || nY != -(int) getY() || index == iterations - 1) {
            if (!isPenUp()) {
          ...
          Double frames = Double.valueOf(1000. / getFramesPerSecond());
          long newTimeStamp = System.currentTimeMillis();
   ...
   * 
   * Negative values for <code>distance</code> are allowed. In that case, the
   * <code>Turtle</code> will move backwards.
   * 
   * @see #bk(double)
   * @return the turtle to allow chaining.
   */
   ...
   * 
   * Negative values for <code>distance</code> are allowed. In that case, the
   * <code>Turtle</code> will move forwards.
   * 
   * @see #fd(double)
   * @return the turtle to allow chaining.
   */
  ...
  /**
   * Method to calculate the number of iterations when animating left or right
   * (rotation).
   */
  private int getAngleIterations(double dAngle) {
    ...
    Double dValue = Double.valueOf(Math.ceil(dAbsAngle / getAngleSpeed() * getFramesPerSecond()));
    return dValue.intValue();
  }

  /**
   * Method to calculate the number of iterations when animating forwards or
   * backwards.
   */
  private int getPathIterations(double length) {
    ...
    Double dValue = Double.valueOf(Math.ceil(dAbsLength / getSpeed() * getFramesPerSecond()));
    return dValue.intValue();
  }

  /**
   * Lifts the <code>Turtle</code>s <code>Pen</code> up so it won't draw a line
   * anymore when moving.
   * 
  ...
  /**
   * Lowers the <code>Turtle</code>s <code>Pen</code> down so it will draw a line
   * when moving.
   * 
   ...
   * 
   * @return <code>true</code> if the <code>Pen</code> is up, <code>false</code>
   *         otherwise.
   * @see #pu()
   * @see #pd()
   */
  ...
  /**
   * Returns the bounds of this <code>Turtle</code>. This is required by the
   * methods that (return-)paint the Turtles.
   */
  Rectangle getBounds() {
  ...
  /**
   * Move the Turtle back "home", i.e. set its position to the origin, facing
   * NORTH.
   * 
  ...
  /**
   * The <code>Turtle</code>s <code>Pen</code> is changed to an eraser (which is
   * in fact a pen with background color).
   * 
   ...
   * 
   * @return the direction from the current turtle position towards the given
   *         point, measured in degrees and clockwise from the vertical upwards
   *         position.
   */
  ...
  /**
   * Puts the Turtle to the top (i.e. above any other Turtle).
   * 
   ...
   * 
   * It could be that the turtle moves beyond the edge of the window. You won't
   * see it anymore unless it finds its way back :-)<br>
   ...
   * 
   * A Region is bounded by lines of any other color than the background color and
   * by the border of the Window. <br>
   * 
   * @return the turtle to allow chaining.
   */
  public Turtle fill() {
  ...
  /**
   * Clears away all that was painted in some way by a turtle (such as lines,
   * fillings, text, stamps etc.)
   * 
   * @return the turtle to allow chaining.
   */
  public Turtle clean() {
  ...
  /**
   * Change the current font to the specified one. If you want to know what fonts
   * are available on your system, call #getAvailableFontFamilies() .
   * 
   ...
   * 
   * This is either
   * <code>java.awt.Font.PLAIN, java.awt.Font.BOLD, java.awt.Font.ITALIC</code> or
   * <code>java.awt.Font.BOLD+java.awt.Font.ITALIC</code>
   * 
  ...
  /**
   * Provides information about all font families (e.g. Roman) currently available
   * on your system.
   * 
  ...
  //Custom methods added to faclitate drawing of curves that mimic java.awt

  public Turtle drawOval(double c1, double c2, double r1, double r2) {
    double startAngle = 0;
    double arcAngle = 360;
    double a = c2 + r1 / 2;
    double b = c1 - r2 / 2;
    double angle = startAngle;
    setPos(a + ((r1 / 2) * Math.cos(Math.toRadians(angle))), b + ((r2 / 2) * Math.sin(Math.toRadians(angle))));
    while (angle <= arcAngle + startAngle) {
      double x = a + ((r1 / 2) * Math.cos(Math.toRadians(angle)));
      double y = b + ((r2 / 2) * Math.sin(Math.toRadians(angle)));
      setH(towards(x, y)).fd(distance(x, y));
      angle += 5;
    }
    return this;
  }

  public Turtle drawArc(double c1, double c2, double r1, double r2, double startAngle, double arcAngle) {
    double a = c2 + r1 / 2;
    double b = c1 - r2 / 2;
    if (arcAngle >= 0) {
      double angle = startAngle;
      ...
      double angle = startAngle + 360;
      setPos(a + ((r1 / 2) * Math.cos(Math.toRadians(angle))), b + ((r2 / 2) * Math.sin(Math.toRadians(angle))));
      while (angle >= arcAngle + 360 + startAngle) {
        double x = a + ((r1 / 2) * Math.cos(Math.toRadians(angle)));
        double y = b + ((r2 / 2) * Math.sin(Math.toRadians(angle)));
        setH(towards(x, y)).fd(distance(x, y));
        angle -= 5;
  ...
  public Turtle drawPoints(Point2D.Double pos, Color color, File file, double s) throws FileNotFoundException {

    setFillColor(color);
    int count = 0;
    try (Scanner sc = new Scanner(file)) {
      Point2D.Double point = new Point2D.Double(pos.x + sc.nextDouble() * s, pos.y + sc.nextDouble() * s);
      setPos(point);
      while (sc.hasNext()) {
        setH(towards(point)).fd(distance(point));
        point = new Point2D.Double(pos.x + sc.nextDouble() * s, pos.y + sc.nextDouble() * s);
        count += 1;
        if (count == 421) {
